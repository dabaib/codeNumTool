...
contextBridge.exposeInMainWorld('svnAPI', {
  ...
  getStats: (projects, username, password, author, year, month, threshold, formatThreshold) =>
    ipcRenderer.invoke('svn-stats', { projects, username, password, author, year, month, threshold, formatThreshold }),
...
contextBridge.exposeInMainWorld('gitlabAPI', {
  ...
  gitlabGetStats: ({ gitlabUrl, projects, author, branches, year, month, threshold, formatThreshold }) =>
    ipcRenderer.invoke('gitlab-stats', { gitlabUrl, projects, author, branches, year, month, threshold, formatThreshold }),
  ...
  apiGetStats: ({ gitlabUrl, token, projectId, projectName, author, branches, year, month, threshold, formatThreshold }) =>
    ipcRenderer.invoke('gitlab-api-stats', { gitlabUrl, token, projectId, projectName, author, branches, year, month, threshold, formatThreshold }),

  // ========== 新增: 本地扫描模式 ==========
  selectLocalRepo: () =>
    ipcRenderer.invoke('select-local-repo'),
  localGetStats: ({ repoPath, repoName, author, branches, year, month, threshold, formatThreshold }) =>
    ipcRenderer.invoke('gitlab-local-stats', { repoPath, repoName, author, branches, year, month, threshold, formatThreshold }),

  // ========== 新增: SSH 远程模式 ==========
  testSshConnection: (repoUrl) =>
    ipcRenderer.invoke('test-ssh-connection', { repoUrl }),
  sshGetStats: ({ repoUrl, repoName, author, branches, year, month, threshold, formatThreshold }) =>
    ipcRenderer.invoke('gitlab-ssh-stats', { repoUrl, repoName, author, branches, year, month, threshold, formatThreshold }),
